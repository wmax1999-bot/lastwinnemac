...
function mapListing(r){return{
  id:String(r.id??r.listingId??r['Listing UID']??''), title:r.title??r.address??r['Address']??'',
  address:r.address??r['Address']??'', unit:r.unit??r['Unit']??'',
  rent:Number(r.rent??r.price??r['Rent']??0),
  beds:r.beds??r['Bedrooms']??null, baths:r.baths??r['Full bathrooms']??null,
  sqft:r.sqft??r.squareFeet??r['Square Feet']??null,
  url:r.url??r.link??r['Short url']??'', availableOn:r.availableOn??r['Available On']??r['Available Date']??null,
...
export async function GET(req){
  ...
  const upstream = await callShowMojo(API_URL, API_KEY);
  const text = await upstream.text();

  if (!upstream.ok) {
    return new Response(JSON.stringify({ error:'ShowMojo error', status: upstream.status, body: text?.slice(0,4000) }), { status:502 });
  }
