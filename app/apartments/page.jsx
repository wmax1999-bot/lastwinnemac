  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <button onClick={()=>{setHood(''); setBeds(''); setPrice('');}} className="border rounded-lg px-3 py-2 hover:bg-gray-50">Reset</button>
      </div>
        ...
        {filtered.map(u => (
          <div key={u.id} className="bg-white rounded-xl shadow hover:shadow-lg overflow-hidden">
            ...
            <div className="p-4">
              ...
              <p className="text-sm text-gray-600">{'{'}u.beds===0? 'Studio' : `${'{'}u.beds{'}'}`} Bed{'}'} • {'{'}u.baths{'}'} Bath • {'{'}u.hood{'}'}</p>
              <div className="mt-4 flex gap-3">
                <a href="https://winne.twa.rentmanager.com/applynow" target="_blank" rel="noreferrer" className="bg-blue-700 text-white px-3 py-2 rounded-md text-sm font-semibold">Apply</a>
                <a href="/contact" className="border border-blue-700 text-blue-700 px-3 py-2 rounded-md text-sm font-semibold">Schedule a Tour</a>
              </div>
            </div>
          </div>
        ))}
